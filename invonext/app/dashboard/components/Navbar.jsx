  ...
  return (
    <header className="w-full bg-white shadow-md  px-4 py-4 md:px-8 flex items-center justify-between">
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <button onClick={toggleFullscreen} className="p-2 rounded-lg hover:bg-gray-100 transition">
          <Maximize2 size={22} />
        </button>
      ...
      >
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            <Image src="/Logo/blog-svgrepo-com (1).svg" width={36} height={36} alt="Invo-Nxt Logo" />
        ...
        <nav className="flex-1 px-4 py-4 space-y-3">
          <Link href={ROUTES.newInvoice} onClick={() => setMobileMenu(false)} className="block text-center bg-indigo-100 text-indigo-600 font-medium py-2 rounded-lg hover:bg-indigo-200">
            + New Invoice
          </Link>
          <hr className="border-gray-200" />
          <h3 className="text-sm font-semibold text-gray-900">Notifications</h3>
          {notifications.map((n) => (
            <Link key={n.id} href={n.link} onClick={() => setMobileMenu(false)} className="block px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700">
              {n.text}
            </Link>
          ))}
          <hr className="border-gray-200" />
